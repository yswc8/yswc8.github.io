...
* **✰ 15-213 Introduction to Computer Systems**: a low-level systems class that covers concepts such as assembly language, memory models, network, and concurrent programming. I have never learnt so much in a class and this is the most engaging cs core for me so far. The labs were extremely beneficial for understanding the course content, and I enjoyed most of them, despite their difficulty. My favourite was malloc lab, which was rewarding, fun, yet so painful to debug. In order of decreasing difficulty, the labs were: Shell (conceptually hard) > Malloc Final (I died from debugging it) > Cache (Start from scratch? No) > Proxy (ahhh the first few traces) > Shark File System (thread-level paralellism was hard to understand for me) > Malloc Checkpoint > Bomb (set the breakpoints!) > Attack > CProgramming. In hindsight, the course was worth the effort and extra money paid this summer. An A is achievable if you put in the effort.  
